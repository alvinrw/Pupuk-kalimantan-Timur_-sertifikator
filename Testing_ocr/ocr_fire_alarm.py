...
def extract_fire_alarm_cert(pdf_path: str) -> dict:
    ...
    try:
        ...
        # 4. Jumlah Detector (e.g. "40 Smoke, 21 Heat & 5 MCP")
        ...
        # 5. Tanggal Terbit & Expired (+1 Tahun)
        issue_date_str = None
        for l in lines:
        ...
        out_file_path = os.path.join(script_dir, "cert_fire_alarm_result.json")
...
if __name__ == "__main__":
    ...
    # Cek jika target file langsung diberikan
    pdf_target = os.path.join(script_dir, "Surat Keterangan", "Fire Alarm System Gedung Arsip Teknik.pdf")
    
    if not os.path.exists(pdf_target):
        # Cari file PDF Fire Alarm apapun di direktori Testing_ocr
        for root, dirs, files in os.walk(script_dir):
            for f in files:
                if "Fire Alarm" in f and f.endswith(".pdf"):
                    pdf_target = os.path.join(root, f)
                    break
            if pdf_target: break

    if pdf_target and os.path.exists(pdf_target):
        res = extract_fire_alarm_cert(pdf_target)
        print(f"SUCCESS: OCR Extractor Fire Alarm Berhasil Memproses: {os.path.basename(pdf_target)}!")
    else:
        print("ERROR: File Fire Alarm PDF tidak ditemukan.")
